...
public class WeightedQuickUnionDS implements DisjointSet {
    ...
    public WeightedQuickUnionDS(int N) {
        ...
        count = N;
        for (int i = 0; i < N; i += 1) {
            parent[i] = -1;              // Using negative value to represent size of a union.
        }                                // Initially each set is one size.
    }
    ...
    /** Returns the total number of unions in UF. */
    public int count() {
        return count;
    }
    ...
    private int find(int v) {
        ...
        int root = v;
        // If parent[root] < 0, then root is itself.
        while (parent[root] >= 0) {
            root = parent[root];
        }
        while (v != root) {
    ...
    public void connect(int v1, int v2) {
        if (!isConnected(v1, v2)) {
            ...
            if (sizeOf(v1) <= sizeOf(v2)) {
                ...
                parent[find(v1)] = find(v2);         // Must compute size before move root to anther set.
            } else {
